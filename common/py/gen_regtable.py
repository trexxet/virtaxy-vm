...
for line in regs_desc:
    ...
    if words[0] == 'Groups':
        ...
        if Public or Private:
            serviceRegs.append([words[0], groupID, bitmask, int(Private)])
            groupID += 1
        else:
            patternList.append([words[0], bitmask, 0])
regs_desc.close()

# Form registers list
matchedList = list(product(groups, patternList))
regList = list(map(lambda x: [x[1][0].replace('?', x[0][1]), x[0][0], x[1][1], x[1][2]],\
